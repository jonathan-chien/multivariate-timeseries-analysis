function mpp = markov_point_process(timeseries, varargin)
...
%       is a (filtered) Markov point process.
...
addParameter(p, 'min_threshold', 0.25, @(x) isscalar(x) && 0 <= x && x <= 1);
addParameter(p, 'zscore', true, @islogical); % Optionally zscore first.
addParameter(p, 'convolve', [0.1 0.2 0.4 0 0]/0.7, @isnumeric);
...
% Optionally convolve to create filtered Markov point process.
if nv.convolve
    for i_region = 1:n_regions
